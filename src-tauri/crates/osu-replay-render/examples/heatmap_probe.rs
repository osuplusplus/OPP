...
fn main() {
    ...
    for w in rep.frames.windows(2) {
        if (w[1].left && !w[0].left) || (w[1].right && !w[0].right) {
            presses.push(Press {
                time: w[1].time,
                pos: [w[1].position.0, w[1].position.1],
            });
        }
    }
    ...
    let cursor_at = |t: f64| -> [f32; 2] {
        ...
        if let Some(next) = snaps.get(idx + 1) {
            ...
            if span > 0.0 && t > snaps[idx].time {
                ...
                return [
                    cur.x + (next.cursor.x - cur.x) * f,
                    cur.y + (next.cursor.y - cur.y) * f,
                ];
            }
        }
        [cur.x, cur.y]
        ...
        match label {
            ...
            _ => match &obj.kind {
                ProcKind::Slider { path, nested, .. } => {
                    ...
                    [
                        obj.position.x + stack.x + p.x,
                        obj.position.y + stack.y + p.y,
                    ]
                }
    ...
    {
        let seq: Vec<String> = engine
            .timeline
            .iter()
            .take(60)
            .map(|e| format!("{}:{}:{:.0}", e.label, e.object_index, e.time))
            .collect();
    ...
    fn find_relative_hit_position(
        ...
        let dist =
            (((hit[0] - next[0]).powi(2) + (hit[1] - next[1]).powi(2)).sqrt() as f64) / radius;
    ...
    fn print_grid(name: &str, points: &[(i32, i32, bool)]) {
        ...
        eprintln!(
            "   mean|rel|={mean:.3} max|rel|={max:.3} share<=0.35: {:.1}%",
            within as f32 / total.max(1) as f32 * 100.0
        );
    }
    ...
    // ---------------- Variant P: current project semantics ----------------
    {
        ...
        for entry in &engine.timeline {
            ...
            if matches!(
                ...
                if let Some(prev) = last {
                    ...
                    if n < 10 {
                        eprintln!(
                            "P sample t={:.0} obj=({}, {}) cursor=({}, {}) dist={:.1} units ({:.2} rel)",
                            entry.time,
                            pos[0],
                            pos[1],
                            hit[0],
        ...
        eprintln!(
            "PROJECT per-event: n={} mean={:.3} median={:.3} p90={:.3} max={:.3}",
            evrels.len(),
            m,
            med,
            evrels[(evrels.len() as f32 * 0.9) as usize],
            evrels[evrels.len() - 1]
        );
    ...
    {
        let judged_end = |obj: &osu_replay_judge::process::ProcObject,
                          label: &str,
                          snap_snaps: &_|
         -> [f32; 2] {
            let _ = snap_snaps;
    ...
    // ---------------- Variant O: official lazer semantics ----------------
    {
                        ...
                        let inwin: Vec<String> = presses
                            ...
                            .map(|p| {
                                format!(
                                    "t={:.0} d={:.1}",
                                    p.time,
                                    (p.pos[0] - this_end[0]).hypot(p.pos[1] - this_end[1])
                                )
                            })
                            .collect();
                        eprintln!(
                            "O dbg t={:.0} obj=({}, {}) t0={:.0} nwin={:?} [{}]",
                            entry.time,
                            this_end[0],
                            this_end[1],
                            t0,
        ...
        eprintln!(
            "CHAIN: {chain_diff}/{chain_events} circle events have start >5 units apart between project/official chains"
        );
        evrels.sort_by(|a, b| a.total_cmp(b));
        let m = evrels.iter().sum::<f32>() / evrels.len() as f32;
        let med = evrels[evrels.len() / 2];
        eprintln!(
            "OFFICIAL per-event hits: n={} mean={:.3} median={:.3} p90={:.3} max={:.3}",
            evrels.len(),
            m,
            med,
            evrels[(evrels.len() as f32 * 0.9) as usize],
