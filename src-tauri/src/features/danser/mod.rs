...
use crate::{
    domain::DanserRenderPreferences,
...
pub fn resolve_ffmpeg_path(
    ...
    if let Some(p) = crate::infrastructure::platform::find_in_path("ffmpeg")
...
fn ensure_opp_profile(
    ...
    let config_dir = crate::infrastructure::platform::danser_config_dir()
...
fn stage_lazer_task(
    ...
    let (songs_dir, skins_root, staged_skin) =
        local_analysis.stage_lazer_danser_resources(&beatmap_md5, Some(&task.preferences.skin))?;
...
pub fn open_danser_output(path: String, state: State<'_, AppState>) -> CommandResult<()> {
    ...
    crate::infrastructure::platform::reveal_path(&target)
