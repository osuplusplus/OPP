...
fn danser_in_path() -> Option<PathBuf> {
    crate::infrastructure::platform::find_in_path("danser")
        .or_else(|| crate::infrastructure::platform::find_in_path("danser-cli"))
...
pub(super) fn list_profiles_for(executable: &Path) -> Vec<String> {
    ...
    let settings_dir = {
        let _ = executable;
        crate::infrastructure::platform::danser_config_dir()
    };
    #[cfg(windows)]
...
pub(super) fn ffmpeg_available(executable: &Path) -> bool {
    ...
    bundled || crate::infrastructure::platform::find_in_path("ffmpeg").is_some()
}
