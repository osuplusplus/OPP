...
fn client_running(_client: LocalClient, executable: &Path) -> bool {
    ...
    {
        let _ = executable;
        crate::infrastructure::platform::game_process_running(&_client.to_string())
    }
}
...
pub(crate) fn executable(client: LocalClient, root: &str) -> Option<PathBuf> {
    ...
    {
        let _ = root;
        crate::infrastructure::platform::game_command(&client.to_string()).map(PathBuf::from)
    }
    #[cfg(windows)]
    {
...
fn scan_game_status(
    local_analysis: &crate::features::local_analysis::LocalAnalysisService,
) -> GameStatusSnapshot {
    #[cfg(not(windows))]
...
pub fn start_game_monitor(
    local_analysis: Arc<crate::features::local_analysis::LocalAnalysisService>,
