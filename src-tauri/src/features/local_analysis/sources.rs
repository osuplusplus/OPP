...
fn resolve_stable(configured_path: Option<&Path>) -> ResolvedSource {
    ...
    let detected = configured_path.map(Path::to_path_buf).or_else(|| {
        crate::infrastructure::platform::stable_install_candidates()
            .into_iter()
...
fn resolve_lazer(configured_path: Option<&Path>) -> ResolvedSource {
    resolve_lazer_with_ini(
        configured_path,
        crate::infrastructure::platform::resolve_lazer_data_root(),
    )
}
...
fn resolve_lazer_with_ini(
    ...
    let registry_install = crate::infrastructure::platform::lazer_install_candidates()
        .into_iter()
...
mod tests {
    ...
    fn validates_stable_and_lazer_features() {
        ...
        assert!(
            resolve_lazer_with_ini(Some(lazer.path()), None)
                .status
                .valid
        );
    }
