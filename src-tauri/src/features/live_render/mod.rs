    ...
    match cmd {
        ...
        Cmd::Move { rect } => {
            ...
            if let Some(s) = session.as_mut() {
                ...
                if (rect.width <= 0.0 || rect.height <= 0.0) && s.playing {
                    s.playing = false;
                    s.audio_pause();
                }
            }
        ...
        Cmd::SetOptions(options) => {
            ...
            if options.storyboard != s.storyboard || options.video != s.video {
                ...
                let (atlas, fonts) =
                    build_atlas(bg_image, avatar_image, &mut new_skin, 8192, slots);
                {
                    let Backend::Native { renderer, .. } = &mut s.backend;
            ...
            if (options.bg != s.has_bg || avatar_changed) && options.skin_path == s.skin_path {
                ...
                match skin::load_skin(s.skin_path.as_deref().map(std::path::Path::new)) {
                    Ok(mut skin) => {
                        ...
                        let built = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                            let (atlas, fonts) =
                                build_atlas(bg_image, avatar_image, &mut skin, 8192, None);
                            s.set_atlas(&atlas);
                            s.atlas = atlas;
                            s.fonts = fonts;
                            s.skin = skin;
...
mod repro_tests {
    ...
    #[test]
    #[ignore = "requires an explicit local Songs and Replays corpus"]
...
pub fn live_render_get_ffmpeg_status(
    ...
    } else if let Some(p) = crate::infrastructure::platform::find_in_path("ffmpeg")
        .or_else(|| crate::infrastructure::platform::find_in_path("ffmpeg.exe"))
    {
        source = "path";
        Some(p)
    } else {
        danser
            .as_deref()
            .and_then(|d| crate::features::danser::resolve_ffmpeg_path(None, Some(d)))
            .inspect(|_| {
                source = "danser";
            })
    };
