...
import { ChevronDown, CircleStop, FolderOpen, Gauge, LoaderCircle, Play, RefreshCw, Settings2, Terminal } from "lucide-react";
...
import type { DanserRenderJob, DanserRenderPreferences, DanserStatus } from "../../shared/types/osu";
import { settingsQueryKey, useSettings } from "../settings/api";
import { useReplayWorkspace } from "./api";
...
export function DanserRenderPanel() {
  ...
  const { replayPath, replayInfo, inspectError, checked } = useReplayWorkspace();
  const blocked = replayBlockReason(replayPath, replayInfo, "danser", inspectError);
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const [nextStatus, queue] = await Promise.all([
        desktopApi.getDanserStatus(), desktopApi.getDanserRenderQueue(),
      ]);
  ...
  const enqueue = async () => {
    const paths = checked.length ? checked : replayPath ? [replayPath] : [];
    if (!paths.length) return;
    setBusy(true); setError(null);
    try {
      await savePreferences();
      const created = await desktopApi.enqueueDanserRenders({ client, replay_paths: paths, preferences });
      setJobs((current) => {
  ...
  return <div className="studio-panel">
    {error ? <ErrorPanel error={error} onRetry={() => void refresh()} /> : null}
    <div className="studio-grid"><ReplayIdentity provider="danser"><p className="studio-provider-note">在本机生成视频 · {waitingCount} 个任务等待开始</p></ReplayIdentity>
    <aside className="studio-settings" aria-label="Danser 渲染设置"><div className="studio-settings-scroll">
      <details className="studio-environment"><summary>本地运行环境 · {status?.available && status.ffmpeg_available ? "已就绪" : "需配置"}</summary>        <Card className="p-5">
      ...
      <section className="studio-output"><h3>输出位置</h3><p>{stored.data?.replay_export_directory ?? "尚未设置"}</p><Button onClick={() => void chooseOutput()} size="sm"><FolderOpen />选择目录</Button></section>
        <Card className="p-5"><SectionTitle title="视频质量" description="任务在加入队列时保存当前画质设置。" />
        ...
        <Card className="p-5"><SectionTitle title="皮肤与播放" description="选择皮肤、调整速度，或仅导出回放中的一段。" />
          <div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
