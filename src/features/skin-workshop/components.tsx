...
export function LazerReadOnly({ skins, selected, onSelect }: { skins: LocalSkinSummary[]; selected: string | null; onSelect: (id: string) => void }) {
  ...
  return <div className="grid min-h-0 grid-cols-[320px_minmax(0,1fr)] gap-4"><Card className="min-h-0 max-h-[calc(100vh-250px)] overflow-y-auto overscroll-contain p-3">{skins.map((skin) => <button className={cn("mb-2 w-full rounded-xl border p-3 text-left", selected === skin.resource.resource_id ? "border-amber-300/30 bg-amber-300/10" : "border-white/[0.06] bg-white/[0.02]")} key={skin.resource.resource_id} onClick={() => onSelect(skin.resource.resource_id)} type="button"><p className="truncate text-sm font-semibold text-white">{skin.name}</p><p className="mt-1 text-[10px] text-slate-500">{skin.author} · {skin.version}</p></button>)}</Card><Card className="min-h-0 max-h-[calc(100vh-250px)] overflow-y-auto overscroll-contain p-5"><Badge tone="warning">Lazer 只读</Badge><h2 className="mt-4 text-xl font-semibold text-white">{detail.data?.summary.name ?? "选择一套 Skin"}</h2><p className="mt-2 text-sm leading-6 text-slate-400">当前界面仅展示 lazer 皮肤配置，暂不支持组件融合和原地修改。已安装皮肤可用于渲染预览。</p><div className="mt-5 space-y-2">{detail.data?.sections.map((section) => <details className="rounded-xl border border-white/[0.07] p-3" key={section.name}><summary className="cursor-pointer text-xs font-semibold text-slate-200">[{section.name}]</summary><div className="mt-3 space-y-2">{section.entries.map((entry, index) => <div className="grid grid-cols-[140px_1fr] gap-3 text-xs" key={`${entry.key}:${index}`}><span className="font-mono text-slate-600">{entry.key}</span><span className="break-all text-slate-300">{entry.value}</span></div>)}</div></details>)}</div></Card></div>;
}
