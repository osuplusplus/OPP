...
export function OnlineStageToolbar({ query, ruleset, items, text, onTextChange, onApply, onClear }: {
  ...
  return <Dialog.Root open={filtersOpen} onOpenChange={setFiltersOpen}>
    <form className="online-search" data-page-guide-online-search="true" onSubmit={(event) => { event.preventDefault(); dismissSuggestions(); onApply({ ...query, query: text.trim(), sort: text.trim() ? "relevance_desc" : "ranked_desc" }); }}>
