    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-[minmax(0,1fr)_clamp(14rem,20vw,17rem)] items-start gap-5">
        <section className="opp-online-results min-w-0" data-page-guide-online-results="true">
          ...
          {search.isLoading ? <div className="opp-online-results-grid">{Array.from({ length: 6 }, (_, index) => <Skeleton className="opp-beatmap-card-skeleton rounded-xl" key={index} />)}</div> : search.error ? <ErrorPanel error={search.error} onRetry={() => search.refetch()} /> : !items.length ? <EmptyState action={<Button onClick={reset}><Music2 className="size-4" />查看近期 Ranked</Button>} description="请放宽筛选条件，或更换内容筛选标签。" icon={<SearchX className="size-5" />} title="没有找到匹配的谱面" /> : <>
            {/* 按结果区的实际宽度切换列数，避免两侧栏占位后仍按窗口宽度强制双列。 */}
            <div className="opp-online-results-grid">{items.map((beatmapset) => <BeatmapsetCard beatmapset={beatmapset} downloading={directDownloadId === beatmapset.id} key={beatmapset.id} onAddToCollection={() => openCollectionDialog(collectionCandidates(beatmapset))} onDownload={() => void downloadBeatmapset(beatmapset)} onOpen={() => setManualDetailId(beatmapset.id)} onPreview={() => togglePreview(beatmapset)} onSelect={() => toggleQueue(beatmapset)} playing={playingId === beatmapset.id} selected={queue.has(beatmapset.id)} />)}</div>
