    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-[minmax(0,1fr)_286px] items-start gap-5">
        <section className="min-w-0" data-page-guide-online-results="true">
          ...
          <div className="opp-beatmap-toolbar mb-4 flex min-h-12 items-center justify-between px-4">
          ...
          {search.isLoading ? <div className="opp-beatmap-grid">{Array.from({ length: 6 }, (_, index) => <Skeleton className="h-[220px] rounded-xl" key={index} />)}</div> : search.error ? <ErrorPanel error={search.error} onRetry={() => search.refetch()} /> : !items.length ? <EmptyState action={<Button onClick={reset}><Music2 className="size-4" />查看近期 Ranked</Button>} description="请放宽筛选条件，或更换内容筛选标签。" icon={<SearchX className="size-5" />} title="没有找到匹配的谱面" /> : <>
            {/* 自动列宽保证宽窗口一行多张卡片，窄窗口自然回落为单列。 */}
            <div className="opp-beatmap-grid">{items.map((beatmapset) => <BeatmapsetCard beatmapset={beatmapset} downloading={directDownloadId === beatmapset.id} key={beatmapset.id} onAddToCollection={() => openCollectionDialog(collectionCandidates(beatmapset))} onDownload={() => void downloadBeatmapset(beatmapset)} onOpen={() => setManualDetailId(beatmapset.id)} onPreview={() => togglePreview(beatmapset)} onSelect={() => toggleQueue(beatmapset)} playing={playingId === beatmapset.id} selected={queue.has(beatmapset.id)} />)}</div>
