...
function OnlineBeatmapsClient({ ruleset }: { ruleset: Ruleset }) {
  ...
  const [previewBeatmapset, setPreviewBeatmapset] = useState<{ beatmaps: Array<{ id: number; version: string; mode: string; difficulty_rating: number }> } | null>(null);
  ...
  return <>
      ...
      <div className="grid grid-cols-[minmax(0,1fr)_clamp(14rem,20vw,17rem)] items-start gap-5">
        <section className="opp-online-results min-w-0" data-page-guide-online-results="true">
          ...
          {search.isLoading ? <div className="grid grid-cols-1 gap-3.5 lg:grid-cols-2">{Array.from({ length: 6 }, (_, index) => <Skeleton className="aspect-[136/55] rounded-xl" key={index} />)}</div> : search.error ? <ErrorPanel error={search.error} onRetry={() => search.refetch()} /> : !items.length ? <EmptyState action={<Button onClick={reset}><Music2 className="size-4" />查看近期 Ranked</Button>} description="请放宽筛选条件，或更换内容筛选标签。" icon={<SearchX className="size-5" />} title="没有找到匹配的谱面" /> : <>
            ...
            <div className="grid grid-cols-1 gap-3.5 lg:grid-cols-2">{items.map((beatmapset) => <BeatmapsetCard beatmapset={beatmapset} downloading={directDownloadId === beatmapset.id} key={beatmapset.id} onAddToCollection={() => openCollectionDialog(collectionCandidates(beatmapset))} onDownload={() => void downloadBeatmapset(beatmapset)} onOpen={() => setManualDetailId(beatmapset.id)} onPreview={() => togglePreview(beatmapset)} onVisualPreview={() => { const previewBeatmapset = { beatmaps: beatmapset.beatmaps?.map((b) => ({ id: b.id, version: b.version, mode: b.mode, difficulty_rating: b.difficulty_rating })) ?? [] }; setPreviewBeatmapset(previewBeatmapset); }} onSelect={() => toggleQueue(beatmapset)} playing={playingId === beatmapset.id} selected={queue.has(beatmapset.id)} />)}</div>
    ...
    <BeatmapsetDetailDialog beatmapsetId={detailId} fallback={detailFallback} initialBeatmapId={deepLink.beatmapId} key={detailId ?? "closed"} onAddToCollection={(beatmapset) => openCollectionDialog(collectionCandidates(beatmapset))} onClose={closeDetail} onFindSimilar={(beatmapId, beatmapRuleset) => navigate(similarityRouteForBeatmap(beatmapId, beatmapRuleset))} onPreview={togglePreview} onVisualPreview={(beatmapset) => { const previewBeatmapset = { beatmaps: beatmapset.beatmaps?.map((b) => ({ id: b.id, version: b.version, mode: b.mode, difficulty_rating: b.difficulty_rating })) ?? [] }; setPreviewBeatmapset(previewBeatmapset); }} playing={detailId !== null && playingId === detailId} />
    {previewBeatmapset ? <div className="fixed inset-0 z-[120] overflow-y-auto bg-black/70 p-5 backdrop-blur-md" onClick={() => setPreviewBeatmapset(null)}><div className="mx-auto mt-8 max-w-4xl" onClick={(event) => event.stopPropagation()}><BeatmapPreviewCard embeddedBeatmapset={previewBeatmapset} /></div></div> : null}
  </>;
}
